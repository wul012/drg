...
int main() {
    ...
    for (int i = 0; i < testTime; i++) {
        ...
        // 2. 复制多份，用于不同的排序算法
        std::vector<int> arr1 = originalArr; // 用于选择排序
        std::vector<int> arr2 = originalArr; // 用于标准库排序 (对照组)
        std::vector<int> arr3 = originalArr; // 用于快速排序

        // 3. 分别用三种方法进行排序
        selectionSort::sort(arr1); // 使用我们自己实现的 selectionSort
        comparator(arr2);          // 使用标准库的 std::sort 作为对照
        quickSort::sort(arr3);     // 使用我们自己实现的 quickSort

        // 4. 比较排序结果
        if (!areVectorsEqual(arr1, arr2) || !areVectorsEqual(arr3, arr2)) {
            // 如果有任何一个算法的结果与标准库不一致，说明有 bug
            succeed = false;
            ...
            if (!areVectorsEqual(arr1, arr2)) {
                std::cout << "--> selectionSort 结果错误: ";
                printVector(arr1);
            }
            if (!areVectorsEqual(arr3, arr2)) {
                std::cout << "--> quickSort 结果错误: ";
                printVector(arr3);
            }
    ...
    // --- 单次运行演示 (selectionSort) ---
    std::cout << "\n--- 单次运行演示 (selectionSort) ---" << std::endl;
    ...
    // --- 单次运行演示 (quickSort) ---
    std::cout << "\n--- 单次运行演示 (quickSort) ---" << std::endl;
    std::vector<int> arrForQuickSort = generateRandomVector(maxSize, maxValue);
    std::cout << "排序前: ";
    printVector(arrForQuickSort);
